...
void ADC0_init(){
    ...
    //reset 9th, 10th, and set 8th
    PORTB->PCR[1] |= (1<<8); //set 8th
    PORTB->PCR[1] &= 0xFFFFF9FF; //reset 9th , 10th bit to 0
...
int main(void){
    ...
    while(1){
        ...
        result = ADC0->R[0] & 0xFFF; //0 for A  //FFF => 3*4 => (1<<12) as below
